...
class Ajax extends CI_Controller {
	...
	public function get_customer_list() {
		...
		$items = $this -> customer_model -> findPageItems($offset, $items_per_page, $user_id, $group_id);
		$total = $this -> customer_model -> countAll($user_id, $group_id);
		// Paging
		$page_link = get_link_pagination('', $total, $offset, $items_per_page, TRUE);
		$data['dataItem'] = $items;
		$html['paging_link'] = $page_link;
		if (empty($group_id)) {
			...
			$html['item_list'] = $this -> load -> view('admin/vwGroupCustomerList', $data, TRUE);
		}
		echo json_encode($html);
		die();
	}		
